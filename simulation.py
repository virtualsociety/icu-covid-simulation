"""
ICU COVID simulation model with probabilities 
...
#Set up lists and dataframes for the data collection
...
def generate_patient(env, arrival_stream, arrival_rate, inital_delay = 0,
    ...
    while (env.now <stoptime):
        ...
        positive_prob = random.random()
        ICU_prob = random.random()
        if (positive_prob > COVID_positive):
            ppi = patient_stream_negative(env, 'Patient number: {}'.format(number_of_patients), 
                                          los_test = los_test) 
            env.process(ppi)
            yield env.timeout(inter_arrival_time)
        elif (positive_prob <= COVID_positive and ICU_prob > ICU_admission):
            ppi = patient_stream_positive(env, 'Patient number: {}'.format(number_of_patients), 
                                          los_test = los_test) 
            env.process(ppi)
            yield env.timeout(inter_arrival_time)
        elif (positive_prob <= COVID_positive and ICU_prob < ICU_admission):
...
def patient_stream_negative(env, name_patient, los_test):
    #COVID testing
    ...
    print("{} is tested NEGATIVE and leaving COVID testing at Day {:.4f}".format(name_patient, env.now))
    patient_number_test_discharge_list.append(name_patient)
    timestamp_test_discharge_list.append(env.now)
    
def patient_stream_positive(env, name_patient, los_test):
    #COVID testing
    print("{} is registering for COVID testing at Day {:.4f}".format(name_patient, env.now))
    patient_number_test_register_list.append(name_patient)
    timestamp_test_register_list.append(env.now)
    ...
    if (env.now > bed_request_timestamp):
        ...
        yield env.timeout(los_test)
    test_unit.release(bed_request1)
    print("{} is tested POSITIVE and leaving COVID testing at Day {:.4f}. MILD SYMPTOMS.".format(name_patient, env.now))
    patient_number_test_discharge_list.append(name_patient)
    timestamp_test_discharge_list.append(env.now)
...
def patient_stream(env, name_patient, los_test, los_icu, los_reg):
    ...
    yield bed_request2
    test_unit.release(bed_request1)
    print("{} leaving COVID testing at Day {:.4f}. SEVERE SYMPTONS".format(name_patient, env.now))
    patient_number_test_discharge_list.append(name_patient)
    timestamp_test_discharge_list.append(env.now)
